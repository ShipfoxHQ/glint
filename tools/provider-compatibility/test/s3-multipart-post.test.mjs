...
test('accepts the AWS signer field casing without changing submitted fields', async () => {
  const client = new S3Client({
    region: 'eu-central-1',
    credentials: {accessKeyId: 'example', secretAccessKey: 'example-secret'},
  });
  const signed = await createPresignedPost(client, {
    Bucket: 'example-bucket',
    Key: 'provider-compatibility/example.png',
    Expires: 300,
    Fields: {'Content-Type': 'image/png'},
    Conditions: [
      ['eq', '$key', 'provider-compatibility/example.png'],
      ['eq', '$Content-Type', 'image/png'],
      ['content-length-range', 1, MAX_UPLOAD_BYTES],
    ],
  });

  assert.ok(Object.hasOwn(signed.fields, 'Policy'));
  assert.ok(Object.hasOwn(signed.fields, 'X-Amz-Algorithm'));
  assert.doesNotThrow(() => validateSignedPost(signed));
});
...
test('rejects PUT-only, incomplete, or unconstrained signed operations', () => {
  ...
  assert.throws(
    () =>
      validateSignedPost({
        fields: {...fixture.responseItem.fields, Policy: 'duplicate'},
        url: 'https://uploads.invalid',
      }),
    /duplicate case-insensitive field/,
  );
});
